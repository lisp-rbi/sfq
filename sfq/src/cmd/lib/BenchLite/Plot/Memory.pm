...
sub new {
    ...
    my $self->{_R_}  = ();
    $self->{_x_scale_} = 'log10'; #log10
    $self->{_y_scale_} = 'log10'; #log10
    $self->{_x_unit_}  = 'MB';
    $self->{_y_unit_}  = 'MB';
...
sub plot {
  ...
  my ($self,$s,$p,$lines,$data,$title) = @_;
    ...
    foreach my $line (@{$lines}){
      ...
      for (my $i=0; $i < @{$line}; $i++){
        my @sel = ();
        for(my $j = 0; $j< @{$data->{'memory'}->{'logic'}}; $j++){
          if ($data->{'memory'}->{'logic'}->[$j]->[$i] eq $line->[$i] || $line->[$i] eq '-'){
            push (@sel, $j);
          }
        }
        if (@selection > 0) {
      ...
      foreach my $l (sort {$a<=>$b} @selection){
        push(@name,join("_", @{$data->{'memory'}->{'logic'}->[$l]}));
        push(@yval, $data->{'memory'}->{'data'}->[$l]->[4]/$self->{_y_unit_div_});
        push(@ysd,  $data->{'memory'}->{'data'}->[$l]->[5]/$self->{_y_unit_div_});
    ...
    $self->{_R_}->run("mdata <- data.frame($group_by, $x, $x\_sd, $y, $y\_sd)");
    ...
    $self->{_R_}->run("m$s <- make_memory_plot(mdata)"); #
...
sub plot_summary {
  ...
  my $Rcode = << "R";
  library(\"ggpubr\")
  mm <- ggarrange($in, ncol=$col, nrow=$row, align = \"v\", common.legend = TRUE, legend=\"bottom\")
  mf <- annotate_figure(mm,top = text_grob(\"Memory Usage\", face = \"bold\", size = 16))
R
if ($self->{_IMG_} eq 'svg' || $self->{_IMG_} eq 'all'){
  $Rcode .= << "R";
  svg(\"Memory.svg\",width=$width, height=$highth)
  mf
  dev.off()
R
}
if ($self->{_IMG_} eq 'pdf' || $self->{_IMG_} eq 'all'){
  $Rcode .= << "R";
  pdf(\"Memory.pdf\",width=$width, height=$highth)
  mf
  dev.off()
R
}
...
sub _make_plot_obj {
  ...
  make_memory_plot <- function(tdf) {
    wdth <- (max(tdf\$$x) - min(tdf\$$x)) * 0.02
    mp <-ggplot(tdf, aes(x=$x, y=$y, group=$group_by, color=$group_by)) +
